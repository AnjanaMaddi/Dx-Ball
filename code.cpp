...
brick_coords brick_array[5][11];
GLfloat px=0, bx = 0, by = -12.8, speed = 0, dirx = 0, diry = 0, start = 0;
	...
	if (start == 0)
	{
		for (i = 1; i <= rows; i++)
		{
			for (j = 1; j <= columns; j++)
			{
				...
				brick_array[i][j].x = (GLfloat)(j * 3.36);
				brick_array[i][j].y = (GLfloat)(i * 1.2);
			}
		}
	}
...
void idle()
{
	...
	bx += dirx / (7*rate);
	by += diry / (7*rate);
	//rate -= 0.0005; // Rate at which the speed of ball increases
